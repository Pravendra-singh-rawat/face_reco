...
def find_matches(video_path, target_image_path):
    ...
    matches_log = []
    frame_count = 0
    last_match_time = -10  # Ensure the first match is recorded
    ...
    while cap.isOpened():
        ...
        if des2 is not None:
            ...
            if len(matches) > 100:
                ...
                # Capture only if 10 seconds have passed since the last match
...
if video_file and image_file:
    with st.spinner("Processing... This may take a while."):
        ...
        if match_results:
            ...
            # Display matches
            for match in match_results:
                st.image(match['Capture Path'], caption=f"Time: {match['Time (s)']}s | Matches: {match['Match Count']}", use_column_width=True)
